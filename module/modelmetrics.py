def model_metrics(model_list=None, metric_list=None, data=None, target=None, class_type=None, test=False):
    '''
    **Warning:
    ...
    if (class_type.lower() == 'classification') or (class_type.lower() == 'classif'):
        if test == False:
            ...
            for i in range(len(model_list)):
                dictionary[str(model_list[i]).split('(')[0]] = list()
                for j in range(len(metric_list)):
                    y_pred = model_list[i].fit(data, target).predict(data)
                    dictionary[str(model_list[i]).split('(')[0]].append(round(metric_list[j](target, y_pred), 2))
            ...
            for i in range(len(model_list)):
                dictionary[str(model_list[i]).split('(')[0]] = list()
                for j in range(len(metric_list)):
                    y_pred = model_list[i].predict(data)
                    dictionary[str(model_list[i]).split('(')[0]].append(round(metric_list[j](target, y_pred), 2))
